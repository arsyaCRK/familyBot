...
func main() {
	...
	for update := range updates {
		...
		if update.Message.IsCommand() {
			...
			case BallyPlus:
				if update.Message.From.UserName == "Hbmkhg" || update.Message.From.FirstName == "Богданыч" {
...
func sendStartMenu(bot *tgbotapi.BotAPI, chatID int64) {
	msg := tgbotapi.NewMessage(chatID, "Привет! Я ваш новый семйный Telegram-бот для удобвтсва работы "+
		"с заданиями и начилсения баллов. 😉. Выберите действие:")
